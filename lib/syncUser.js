 ...
 * This is a fallback function called from API routes to ensure user exists in DB
 */
...
export async function getOrCreateUser(userId) {
  try {
    ...
    if (!user) {
      ...
      // The webhook should have created this, but this is a fallback
      user = await User.create({
        clerkId: userId,
        email: `user_${userId}@placeholder.com`, // Will be updated by webhook
        name: "User",
      });
      console.log("⚠️ Created placeholder user for:", userId);
    }
