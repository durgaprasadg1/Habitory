...
import { buildUserCacheKey, getJsonCache, setJsonCache } from "@/lib/redis";

const HISTORY_CACHE_TTL_SECONDS = 600;
...
export async function GET(request) {
  try {
    ...
    const cacheKey = buildUserCacheKey(
      userId,
      "history",
      requestedYear || "all",
      requestedMonth || "all",
    ...
    if (!user) {
      const responsePayload = { availableMonths: [], monthData: null };
      await setJsonCache(cacheKey, responsePayload, HISTORY_CACHE_TTL_SECONDS);
    ...
    const responsePayload = { availableMonths, monthData };
    await setJsonCache(cacheKey, responsePayload, HISTORY_CACHE_TTL_SECONDS);
    return NextResponse.json(responsePayload);
  } catch (error) {
