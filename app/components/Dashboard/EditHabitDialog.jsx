...
export const EditHabitDialog = ({ habit, onUpdate, onDelete }) => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const updated = json?.habit || json;
      const normalized = { ...updated, _id: String(updated._id) };
      toast.success("Habit updated successfully!");
      setOpen(false);
      if (typeof onUpdate === "function") onUpdate(normalized);
    } catch {
      toast.error("Failed to update habit");
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      if (typeof onDelete === "function") {
        // delegate deletion to parent (parent should perform API call and toasting)
        await onDelete(String(habit._id));
        setDeleteDialogOpen(false);
        setOpen(false);
      } else {
        // fallback: perform delete here if parent didn't provide handler
        const res = await fetch(`/api/habits/${habit._id}`, {
          method: "DELETE",
        });
        if (!res.ok) {
      ...
      console.error("EditHabitDialog delete error:", err);
      toast.error("Failed to delete habit");
    } finally {
      setIsDeleting(false);
    }
